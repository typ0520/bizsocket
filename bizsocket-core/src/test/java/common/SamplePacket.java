...
public class SamplePacket extends Packet {
    ...
    public void setResponse(Map<String,String> map) {
        this.content = MockServer.map2json(map);
    }
    ...
    public static SamplePacket build(Packet reusable, BufferedSource reader) throws IOException {
        SamplePacket packet = null;
        if (reusable != null && reusable instanceof SamplePacket) {
            packet = (SamplePacket) reusable;
        }
        else {
